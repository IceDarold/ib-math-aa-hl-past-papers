"""Прогоняет архивный ноутбук B4 целиком: пустым и с эталонными ответами.

Ноутбук архива устроен иначе, чем практикум: в нём нет ни одного разбора,
который проверялся бы отдельно, — вся его правильность в том, что ответ из
раздела Solutions проходит проверку в ячейке. Поэтому и тест такой: он
...
(кроме шести хешей, где ответ неоткуда вывести), так что совпадение здесь
означает согласие markscheme с самой функцией.
"""
import contextlib
import io
import json
...
import build_archive_b4 as gen
...
cells = code_cells()
# первая ячейка — установочная: import из practicum/functions
os.chdir(os.path.join(ROOT, 'practicum', 'functions'))
...
blanks = blank.count('⬜')
t(f'в пустом прогоне {blanks} незаполненных ответов', blanks > 40)
