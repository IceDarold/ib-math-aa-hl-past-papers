"""Прогоняет архивный ноутбук D2 целиком: пустым и с эталонными ответами.

Устроен так же, как check_archive_e1.py, check_archive_e2.py и
check_archive_e3.py, и по той же причине: в архивном ноутбуке нет ни
...
них: verify_event восстанавливает пространство из условий вопроса и
считает по определению, verify_probability складывает веса выписанных
исходов, verify_constants подставляет числа в условия. Совпадение
в этом тесте поэтому означает согласие markscheme с самой задачей,
а не с моей записью — сорок девять раз подряд.
...
import build_archive_d2 as gen
