"""Прогоняет архивный ноутбук D1 целиком: пустым и с эталонными ответами.
...
них: verify_count пересчитывает объекты перебором по описанию из самого
вопроса, verify_count_law сверяет выражение с перебором при малых n,
verify_param_set подставляет найденное n обратно в условие. Совпадение
в этом тесте поэтому означает согласие markscheme с самой задачей,
а не с моей записью — тридцать раз подряд.

Запуск:  python practicum/tests/check_archive_d1.py
"""
import contextlib
import io
import json
...
import build_archive_d1 as gen
