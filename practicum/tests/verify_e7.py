...
t_('1', verify_ode('Задание 1', sin(x - pi/4) + 1, cos(x - pi/4), ic=(3*pi/4, 2)))
t_('2', verify_implicit('Задание 2', Eq(x**2 + y**2, k), Eq(x**2 + y**2, k)))
t_('3', verify_ode('Задание 3', 4 - 2*exp(-x/10), (4 - y)/10, ic=(0, 2)))

a, b = Integer(-4), -4 - pi
t_('4a', check_expr('Задание 4(a), a', a, D['Задание 4(a), a']))
t_('4b', check_expr('Задание 4(b), b', b, D['Задание 4(b), b']))
t_('4whole', verify_ode('Задание 4 целиком', a/(2*x + sin(2*x) + b),
                        y**2*cos(x)**2, ic=(pi/2, 1)))

t_('5', check_expr('Задание 5(b), k', log(9)/10, D['Задание 5(b), k']))  # эквивалентная форма

t_('6', verify_implicit('Задание 6', Eq(x**2 - 2*x*y - y**2, 4),
                        Eq(x**2 - 2*x*y - y**2, 4)))
t_('7', verify_ode('Задание 7', 3*exp(x) - x - 1, y + x, ic=(0, 2)))
t_('8', verify_ode('Задание 8', x**2 - 2*x - 3 + C*exp(-x), x**2 - 5 - y))

# --- задание 9: числа считаем честно, а не берём из решения ---
pts = euler(lambda a_, b_: (4 - b_)/10, 0.0, 2.0, 0.1, 5)
approx = pts[-1][1]
exact = float(4 - 2*exp(Rational(-5, 100)))
...
vx = Function('v')(x)
got = simplify(solve(Eq(diff(vx*x, x), ((vx*x)**2 - 2*x**2)/x**2),
                           diff(vx, x))[0]*x)
want = vx**2 - vx - 2
t_('10b', simplify(got - want) == 0)
print(('✅' if results[-1][1] else '❌'), f'Задание 10(b): x dv/dx = {got}')

# --- задание 11 (на таймере) ---
t_('11a', verify_ode('11(a)', 2*x*sqrt(1 - log(x)), (y**2 - 2*x**2)/(x*y), ic=(1, 2)))
t_('11b', check_set('11(b)', [-sqrt(2), sqrt(2)], D['11(b)']))  # обратный порядок
...
t_('neg1', not verify_ode('минус потерян', 4/(2*x + sin(2*x) - 4 - pi),
                          y**2*cos(x)**2, ic=(pi/2, 1)))
t_('neg2', not verify_ode('потеряно v в подстановке', 3*exp(x) - x, y + x, ic=(0, 2)))
t_('neg4', not check_set('m без минуса', [sqrt(2)], D['11(b)']))
