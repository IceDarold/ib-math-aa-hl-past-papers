...
print('\n=== _blank смотрит внутрь списков ===')
# check_set падал с SympifyError, если в ячейке остался my = [...]:
# снаружи такой список от заполненного неотличим
KEY_SET = digest('|'.join(sorted(sp.srepr(sp.simplify(v)) for v in (1, 5))))
t('заполненный набор проходит', check_set('  набор', [1, 5], KEY_SET))
t('набор с многоточием даёт ⬜ и не падает',
  not check_set('  [...]', [...], KEY_SET))
t('вложенный пропуск тоже виден',
  not check_set('  [1, ...]', [1, Ellipsis], KEY_SET))
t('и в кортеже', not check_set('  (…)', (Ellipsis,), KEY_SET))
