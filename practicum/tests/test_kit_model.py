...
DATA22 = [(0, 15000), (8, 13350)]
# 10122.3 — округление, а не данные условия: такая пара подаётся с явным sf.
PRED22 = [(27, 10122.3)]
...
T('модель из ноября 2022 воспроизводит оба числа условия',
  verify_model('  P(t)', P22, DATA22))
T('и предсказание 10122.3 — при явном sf=3',
  verify_model('  P(27)', P22, PRED22, sf=3))
...
T('время отсчитано от 2022, а не от 2014',
  not verify_model('  t = 19', P22, [(19, 10122.3)], sf=3))
...
T('модель, промахнувшаяся на 1 из 15000, по умолчанию отвергается',
  not verify_model('  +1', P22 + 1, DATA22))
T('а при sf = 3 такой промах пройдёт — три цифры на 15000 это ±150',
  verify_model('  +1 при sf=3', P22 + 1, DATA22, sf=3))
...
T('шесть значащих цифр — значение по умолчанию — уже нет',
  not verify_model('  по умолчанию', P22, [(27, 10100)]))
T('явный допуск tol перебивает sf',
  verify_model('  tol = 50', P22, [(27, 10100)], tol=50))
