"""Прогоняет архивный ноутбук E2 целиком: пустым и с эталонными ответами.

Устроен так же, как check_archive_e1.py, и по той же причине: в архивном
...
Эталоны в ANSWERS взяты из markschemes; шесть проверок сверяются с хешем,
остальные семнадцать эталона не хранят вовсе — verify_maclaurin вычитает
ответ из функции, verify_series_solution подставляет его в уравнение,
verify_terms примеряет границу к соседним членам. Для них совпадение
здесь означает согласие markscheme с самой задачей, а не с моей записью.

Запуск:  python practicum/tests/check_archive_e2.py
"""
import contextlib
import io
import json
...
import build_archive_e2 as gen
