"""Прогоняет архивный ноутбук E1 целиком: пустым и с эталонными ответами.

Устроен так же, как check_archive_c3.py, и по той же причине: в архивном
...
(кроме двух хешей, где ответ — не предел, а постоянная, подобранная под
него), так что совпадение здесь означает согласие markscheme с самой
задачей. В этой теме утверждение сильнее обычного: verify_limit подходит
к пределу лестницей точек и сверять ему не с чем вовсе.

Запуск:  python practicum/tests/check_archive_e1.py
"""
import contextlib
import io
import json
...
import build_archive_e1 as gen
