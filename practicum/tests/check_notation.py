...
#
# Запрет сужен на D2. Там P — не самодельное имя, а функция из kit, и пишется
# она ровно так, как её печатает билет: P(A), P(A & B), P(A, given=B). Запрет
# заводился против буквы, стоявшей за pi; pi давно есть под своим именем, и
# ловить надо голое P, а не вызов. Поэтому `P(` разрешено, а `P` само по себе
# по-прежнему нет.
BANNED = [
    (re.compile(r'\bsp\.'), 'sp.', 'пишите sin(x), pi, sqrt(5) — они приходят из kit'),
    (re.compile(r'\bP\b(?!\s*\()'), 'P как pi', 'pi теперь есть под своим именем'),
