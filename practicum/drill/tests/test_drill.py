...
t('банк собран из готовых практикумов', len(bank['practicums']) == 13)
...
try:
    # B4 в карте есть, но практикум ещё не собран, и в банке его нет.
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    t('карточка открывается у каждого приёма банка', len(every) == 104)
